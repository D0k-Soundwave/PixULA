...
`docs/CURRENT_STATE.md` (18 screen modes, 12 registered tool classes behind 28
...
- 256×192 canvas, 32×24 attribute grid, single attribute byte per cell (FLASH/BRIGHT/PAPER/INK) — the STANDARD_ULA mode, and since Phase 12a one of **18 screen modes** in the `SCREEN_MODES` registry (multicolor 8×4/8×2/8×1, ULAplus, ULAplus 8×1, Timex hi-res 512×192, GigaScreen, MultiGigaScreen 8×4/8×2/8×1, Timex hi-res GigaScreen, ULANext, Layer 2 256/320/640, LoRes, Radastan)
...
| **GigaScreen** (102-colour flicker) | Plus | Phase 12b. Corrected claim: ZX-Paintbrush 2.6.1 has NO GigaScreen support at all (verified against its CHM + exe strings) — this row was aspirational. Ours is a full Plus mode, and since 2026-09-23 one editing surface: every cell holds both screens and the artist paints the four blends a cell can show (Paint grid in the colour rail), while Average / Flicker / A / B only choose what the canvas shows (the RECOIL per-channel-average blend, the two screens alternating, or either alone). It replaced per-layer A/B tags, under which a stroke could land on a screen the canvas was not showing. .img 13824 import+export, .mg (MGH type 8) import, and GIF export emits the two sub-screens as a 2 cs flicker loop |
...
9. ~~**Selection border, magic wand, freeform selection**~~ — done (verified 2026-07-08: `selection-tool.js` ships rectangle/cell/wand/freeform modes; an ellipse mode joined them 2026-07-21, rasterised by the same offscreen-fill trick as the lasso). `grid-overlay.js` originally drew this as animated marching ants incl. the masked variant; **2026-08-19** that animation was dropped in favour of one static, crisp, boundary-traced solid border shared by every selection shape (rect/cell/ellipse/lasso/wand) — a dashed line at native 256x192 res reads as a blurred band once scaled up by zoom, and the animation added nothing a plain line doesn't already show. ZX-PB has none of these, so they extend our lead.
